...
PARAM_DICT={'rh_n':'Высокая влажность','temp_n':'Критическая теспература','wind_gust_spd_n': 'Порывы ветрка', 'clouds_n':'Высокая облачность',
...
class Weather:
    ...
    def normalization(self, start_date, get_data, token):
        ...
        send ={"StartDate": start.strftime('%Y-%m-%d'),"EndDate": end.strftime('%Y-%m-%d'),"LocationIds": [1], "param":"hist"}
