...
PARAM_DICT={'rh_n':'Высокая влажность','temp_n':'Критическая температура','wind_gust_spd_n': 'Порывы ветра', 'clouds_n':'Высокая облачность',
...
class Weather:
    ...
    def normalization(self, start_date, get_data, token, locations):
        ...
        send ={"StartDate": start.strftime('%Y-%m-%d'),"EndDate": end.strftime('%Y-%m-%d'),"LocationIds": [locations], "param":"hist"}
        r = requests.post(os.environ['url_server']+'get/', data=json.dumps(send), headers=headers,
                    verify=False)
    ...
    def analize(self, data, date_start, token):
        ...
        for s in analitics:
            if date_start>winter and date_start<summer:
                    ...
                    if s['temp']<-38 or s['wind_spd'] > 12 or s['wind_gust_spd']>12:
                        if len(s['warning'])>0:
                            s['warning']+=','
    ...
    def get_location(self, token):
        ...
        headers = {'Authorization':
                       'Bearer {}'.format(token),
                   'Content-type': 'application/json'}
        r = requests.post('https://localhost:44321/api/locations/list', data=json.dumps(request), headers=headers,
                          verify=False)
    ...
    def get_data_forecast_day(self, lat, lng, days, locations):
        ...
        for i in range(0,days):
            send = {'rh': data['data'][i]['rh'], 'temp': data['data'][i]['temp'], "locationId": locations,
                ...
                'vis': data['data'][i]['vis'], 'dttm': data['data'][i]['datetime'], 'restriction':'', 'warning':''}
